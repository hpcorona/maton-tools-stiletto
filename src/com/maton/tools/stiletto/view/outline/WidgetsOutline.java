...
import com.maton.tools.stiletto.view.action.NewWidgetAction;
import com.maton.tools.stiletto.view.table.DefaultTable;
import com.maton.tools.stiletto.view.table.WidgetsTable;

public class WidgetsOutline extends DefaultOutline {

	static ImageDescriptor icon = ImageDescriptor.createFromFile(
			WidgetsOutline.class, "ui-button.png");

	protected WidgetsTable table;
	protected WidgetPool pool;

	public WidgetsOutline(ExpandBar parent, int idx, WidgetPool pool) {
		super(parent, idx);
		this.pool = pool;
		item.setText("Widgets");
	...
	protected Control createControl(Composite parent) {
		table = new WidgetsTable(parent, DefaultTable.DEFAULT_TABLE_STYLE, pool);
		...
		toolbar.add(new DeleteAction<Widget>(parent.getShell(), BundleContainer
	...
	protected ToolBarManager createToolBarManager(Composite parent) {
		...
		tbm.add(new NewWidgetAction(parent.getShell(), pool));
